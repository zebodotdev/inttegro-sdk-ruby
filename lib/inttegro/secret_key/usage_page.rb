...
require_relative "usage_row"

module Inttegro
  class SecretKey
    # Typed representation of the usage page object in the Inttegro API.
    #
    ...
    #
    # @!attribute [r] rows
    #   Value of the `rows` field in the Inttegro API payload.
    #
    #   Required in the API payload. Wire name: `rows`.
    #   @return [Array<Inttegro::SecretKey::UsageRow>]
    class UsagePage < T::Struct
      const :number, Integer
      const :size, Integer
      const :count, Integer
      const :total, Integer
      const :has_more, T::Boolean
      const :rows, T::Array[Inttegro::SecretKey::UsageRow]
    end
  end
end
